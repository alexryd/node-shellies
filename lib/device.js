...
class Device extends EventEmitter {
  ...
  static deviceTypeToClass(type) {
    if (type === 'SHSW-1') {
      return Shelly1
    } else if (type === 'SHSW-21' || type === 'SHSW-22' || type === 'SHSW-25') {
      return Shelly2
  ...
  * [Symbol.iterator]() {
  ...
  update(msg) {
    ...
    if (
      (msg.serial && msg.serial !== this._lastSerial) ||
      this.type === 'SHHT-1') {
      // SHHT-1 has a bug where it always sends the same serial
